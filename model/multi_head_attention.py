...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.out_projection = nn.Linear(attention_dim, attention_dim, bias=False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for single_head in self.attention_heads:
            head_score = single_head(embedded)
            scores.append(head_score)

        concated = torch.cat(scores, dim=2)
        out = self.out_projection(concated)
